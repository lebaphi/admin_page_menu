...
export class CategoriesComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		this.categoryChangeSub = this.uiService.categoryListChanged.subscribe((menu: Menu) => {
			this.selectedMenu = menu
			this.categorySub = this.fetchCategory(menu.categoryIds).subscribe(
				(categories: Category[]) => {
					this.dataSource.data = categories
				},
				err => this.showError('Fetching category failed, please try again later'),
			)
		})
	}
	...
	fetchMenu(): Observable<Menu[]> {
		...
		this.menuRef = this.db.collection('menus', ref => ref.where('uid', '==', authData.uid))
		return this.menuRef.snapshotChanges().pipe(
			map(docArray => {
				const mappedMenus = docArray.map(doc => {
					const { categoryIds, name, createdDate, uid } = doc.payload.doc.data() as Menu
					return {
	...
	updatedItem(item: Category): void {
		this.categoriesRef.doc(item.id).set({ category: item.category }, { merge: true })
	}
